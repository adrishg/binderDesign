...
for pdb_file in "$folder_with_pdbs"/*.pdb; do
    ...
    epitope_residues=$(python /share/yarovlab/ahgz/scripts/nanobodies/find_epitope_positions.py \
        --pdb "$pdb_file" \
    ...
    for chain in $(echo "$chains_to_fix" | fold -w1); do
        chain_residues=$(python /share/yarovlab/ahgz/scripts/nanobodies/get_chain_residues.py \
            --pdb "$pdb_file" \
            --chain "$chain")
